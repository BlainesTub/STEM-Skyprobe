...
the right. */
float CURRENTSEALEVELPRESSURE = 1024;
...
/* This float determines the margin of error that the motion checker will use for seeing if the probe is still.
This value is here because the accelerometer will display different values even when it's not in motion.
0.15 seems to be a pretty good estimation, with 0.1 being a bit too low.*/
float accelerometerMOE = 0.15;